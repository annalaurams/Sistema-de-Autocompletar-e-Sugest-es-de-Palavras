...
- ```document.hpp:``` Inclusão de bibliotecas
...
De fora da pasta ```include```contém os arquivos
...
As árvores criadas terão a mesma quantidade de elementos que o valor de ```K```e cada nó é uma posição do heap, com palavra e ocorrência que foram armazenadas neçe.
...
Uma árvore binária é uma estrutura de dados hierárquica composta por nós, em que cada nó pode ter, no máximo, dois filhos: um filho à esquerda e um filho à direita. <br>
 Quando a árvore está vazia, o valor da raiz é NULL. Cada nó de uma árvore binária consiste em três partes:
...
#### Diretrizes desse método no programa
-- Quando o valor for igual a um já inserido na árvore, este valor será inserido a esquerda do nó.

### AVL

A árvore AVL é uma árvore de pesquisa binária aproximadamente balanceada. As diferenças entre as alturas das subárvores esquerda e direita é no máximo 1.
...
#### Diretrizes desse método no programa
-- Quando o valor for igual a um já inserido na árvore, este valor será inserido a direita do nó.
...
- Se tratando de tempo de execução, a diferença entre os algoritmos pode ser mostrada pelo gráfico abaixo, que marca o tempo de construção da estrutura para um texto, no caso, o texto ```filosofia```, buscando 8 palavras.
...
- A diferença é significamente pequena, mas para uma grande entrada de dados, algumas especificidades da estrutura podem ser determinantes no tempo de execução.
<br>
 ## 🕜Tempo de execução 
...
A saída completa do programa está no arquivo ```output.txt``` que contém:<br>
